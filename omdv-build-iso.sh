...
updateSystem() {
	...
	if [ "$ABF" = "1" ]; then
	    ...
	    $SUDO urpmi.addmedia --distrib --mirrorlist http://abf.rosalinux.ru/downloads/cooker/repository/x86_64
	    # end of workaround
	    $SUDO urpmq --list-url
...
createSquash() {
    ...
    $SUDO mksquashfs "$CHROOTNAME" "$ISOROOTNAME"/LiveOS/squashfs.img -comp lz4 -Xhc -no-progress -no-recovery -b 4096
